const name : String = 't_hindered.gd'
const menuname : String = 'Hindered (T)'
...
func _init(args : Array):
	...
	chara = args[0]
	duration = 5*args[1]
	UI.ow_hud.creatureRect.logrect.log_other_text(chara, ' is Hindered !', null,'')
...
func _on_new_round(_character : Creature) :
	if duration <= 0 :
		chara.remove_trait(self)
		return
	chara.used_movepoints = floor(chara.get_movement_left()*0.5)
	duration -= 1
...
func get_info_as_text() -> String :
	return 'Hindered for '+str(duration)+' rounds'
