...
| --- | --- | --- |
| Providence project | Schema `12` | The compatibility exporter reads the canonical project directly. It does not consume Providence's native Realmz compiler output. |
| Remake bundle | Manifest `formatVersion: 3`; Classic/runtime documents use `schemaVersion: 4`, semantic `remake/logic.json` uses schema 6, and `remake/scripts.json` uses schema 3 | Remake accepts only the current pre-release contract before indexing any runtime record. Earlier experimental shapes are regenerated, not migrated. The contract present at the first public release will be published as schema 1. |
...
| Classic save envelope | Schema `8` | Only the current pre-release save schema is accepted. Content-version updates within it require a complete compatible state migration chain. |
