class_name ScenarioGodotServices
extends RefCounted

const SCENARIO_SERVICE_CONTRACT_VERSION := 1
...
var _presentation_services: RefCounted
...
func _init() -> void:
	...
	_presentation_services = PresentationServicesScript.new()
	_presentation_services.configure(self)


func scenario_service_contract_version() -> int:
	return SCENARIO_SERVICE_CONTRACT_VERSION
...
func scenario_port_runtime(port_id: String) -> Object:
	...
	if port_id == "core.presentation":
		return _presentation_services
	return self
...
func _show_text(payload: Dictionary) -> Dictionary:
	return await _presentation_services._show_text(payload)
...
func _show_classic_picture(payload: Dictionary) -> Dictionary:
	return _presentation_services._show_classic_picture(payload)
...
func _alter_party_items_with_identity(
	...
	if use_native_name:
		...
		if names.is_empty():
			return _error("Native-name item mutation cannot resolve an item name")
		result = InventoryRulesScript.alter_named_items(
			party,
			names,
		...
		result = InventoryRulesScript.alter_classic_items(
			party,
			[item_id],
			int(payload.get("maxMatches", 0)),
			int(payload.get("operation", 0)),
	...
	if str(result.get("status", "")) == "error":
		return result
	result["identityMode"] = "native-name" if use_native_name else "scenario-stable"
	_refresh_party_panels(party)
	return result
...
func _play_sound(payload: Dictionary) -> Dictionary:
	return _presentation_services._play_sound(payload)
