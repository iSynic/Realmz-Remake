...
const STONE_TEXTURE_PATHS := [
	"res://shared_assets/UI/StonePatternRect9.png",
	"res://shared_assets/UI/buttonUP.png",
]
...
func _validate_scene(scene_path: String) -> bool:
	...
	var stone_resource_paths: Dictionary = {}
	while not file.eof_reached():
		var line := file.get_line()
		lines.append(line)
		if not line.begins_with("[ext_resource "):
			continue
		for texture_path: String in STONE_TEXTURE_PATHS:
			if not line.contains('path="%s"' % texture_path):
				continue
			var resource_id_match := _resource_id_pattern.search(line)
			if resource_id_match != null:
				stone_resource_paths[resource_id_match.get_string(1)] = texture_path
			break

	if stone_resource_paths.is_empty():
		return true
	...
	for line_index: int in range(lines.size() + 1):
		...
		if line.begins_with("[node ") or line_index == lines.size():
			if not node_texture_path.is_empty() \
					and node_filter != CanvasItem.TEXTURE_FILTER_NEAREST:
				push_error(
		...
		if line.begins_with("texture_filter = "):
			...
			for resource_id: String in stone_resource_paths:
				if line == 'texture = ExtResource("%s")' % resource_id:
					node_texture_path = stone_resource_paths[resource_id]
					break
	return valid
